...
class GetStartedWindow(QMainWindow):
    ...
    def create_widgets(self):
        ...
        logo_label.setPixmap(
            QPixmap(logo_path).scaled(500, 500, Qt.AspectRatioMode.KeepAspectRatio)
        )
    ...
    def open_simulation(self):
        file_path, _ = QFileDialog.getOpenFileName(
            self, "Select file", "", "RCM Files (*.rcm)"
        )
        if file_path:
            global P, comps, selected_comps, allProps, lmopts, opts, NRTL_aij, NRTL_bij, NRTL_cij, TcCel, Pc, omega, antoine_params, PLXANT_params
...
class NewSimulationWindow(QMainWindow):
    ...
    def create_widgets(self):
        ...
        delete_components_button.setMaximumWidth(
            add_components_button.sizeHint().width()
        )
        ...
        self.vapor_pressure_var.currentIndexChanged.connect(
            self.update_vapor_pressure_method
        )
    ...
    def move_to_master(self):
        ...
        if selected_items:
            selected_item = selected_items[0]
            global selected_comps, comps
            selected_comps = np.delete(
                selected_comps, self.selected_comps_list.row(selected_item)
            )
            self.update_components_list()
            ...
            if opts.antMethod == 1 and antoine_params.size:
                if opts.activity == 1:
                    ...
                    QMessageBox.information(
                        self,
                        "Help",
                        "Please ensure all parameters are inserted correctly for your selected fluid package.",
                    )
                ...
                QMessageBox.information(
                    self,
                    "Help",
                    "Please ensure all parameters are inserted correctly for your selected vapor pressure model",
                )
        else:
...
class MakeSimWindow(QMainWindow):
    ...
    def save_variables(self):
        file_path, _ = QFileDialog.getSaveFileName(
            self, "Save Variables", "", "RCM Files (*.rcm)"
        )
        if file_path:
            global P, comps, selected_comps, allProps, lmopts, opts, NRTL_aij, NRTL_bij, NRTL_cij, TcCel, Pc, omega, antoine_params, PLXANT_params
            data = {
...
class antoine_params_input(QWidget):
    ...
    def create_widgets(self):
        ...
        self.table.setHorizontalHeaderLabels(
            [self.antoine_bank[i] for i in range(len(self.antoine_bank))]
        )
        self.table.setVerticalHeaderLabels(
            [self.comps[i] for i in range(len(self.comps))]
        )
    ...
    def populate_table(self):
        for row in range(self.antoine_params.shape[0]):
            for col in range(self.antoine_params.shape[1]):
                self.table.setItem(
                    row, col, QTableWidgetItem(str(self.antoine_params[row, col]))
                )
                ...
                if item is not None and item.text() != "":
                    try:
                        ...
                        QMessageBox.warning(
                            self,
                            "Invalid Input",
                            f"Invalid value at row {row+1}, col {col+1}. Please enter numeric values.",
                        )
                        return
    ...
    def show_help(self):
        QMessageBox.information(
            self,
            "Help",
            "Enter numeric values into the table. Click Save to save the data.",
        )
...
class PLXANT_params_input(QWidget):
    ...
    def create_widgets(self):
        ...
        self.table.setHorizontalHeaderLabels(
            [self.PLXANT_bank[i] for i in range(len(self.PLXANT_bank))]
        )
        self.table.setVerticalHeaderLabels(
            [self.comps[i] for i in range(len(self.comps))]
        )
    ...
    def populate_table(self):
        for row in range(self.PLXANT_params.shape[0]):
            for col in range(self.PLXANT_params.shape[1]):
                self.table.setItem(
                    row, col, QTableWidgetItem(str(self.PLXANT_params[row, col]))
                )
    ...
    def populate_table(self):
        for row in range(self.NRTL_aij.shape[0]):
            for col in range(self.NRTL_aij.shape[1]):
                self.table.setItem(
                    row, col, QTableWidgetItem(str(self.NRTL_aij[row, col]))
                )
    ...
    def populate_table(self):
        for row in range(self.NRTL_bij.shape[0]):
            for col in range(self.NRTL_bij.shape[1]):
                self.table.setItem(
                    row, col, QTableWidgetItem(str(self.NRTL_bij[row, col]))
                )
    ...
    def populate_table(self):
        for row in range(self.NRTL_cij.shape[0]):
            for col in range(self.NRTL_cij.shape[1]):
                self.table.setItem(
                    row, col, QTableWidgetItem(str(self.NRTL_cij[row, col]))
                )
...
class SRK_input(QWidget):
    ...
    def create_widgets(self):
        ...
        self.table.setHorizontalHeaderLabels(
            [self.crit_params_bank[i] for i in range(len(self.crit_params_bank))]
        )
        self.table.setVerticalHeaderLabels(
            [self.comps[i] for i in range(len(self.comps))]
        )
...
class PlotOptsWindow(QMainWindow):
    ...
    def save_linewidth(self):
        ...
        try:
            ...
            QMessageBox.critical(
                self, "Invalid Input", "Please enter a valid number for line width."
            )
    ...
    def save_num_lines(self):
        ...
        try:
            ...
            QMessageBox.critical(
                self,
                "Invalid Input",
                "Please enter a valid interger for the amount of lines to genertate with auto-gen.",
            )
...
class SolverOptsWindow(QMainWindow):
    ...
    def create_widgets(self):
        ...
        self.dxi_entry.returnPressed.connect(self.save_dxi)
    ...
    def save_num_points(self):
        ...
        try:
            ...
            QMessageBox.critical(
                self,
                "Invalid Input",
                "Please enter a valid number for number of points to simulate forward and backwards.",
            )

    def save_dxi(self):
        global opts
        try:
            opts.dxi = float(self.dxi_entry.text())
        except ValueError:
            QMessageBox.critical(
                self,
                "Invalid Input",
                "Please enter a valid number for the objective function tolerance.",
            )
    ...
    def save_maxiter(self):
        ...
        try:
            ...
            QMessageBox.critical(
                self,
                "Invalid Input",
                "Please enter a valid integer for the amount of lines to genertate with auto-gen.",
            )
    ...
    def save_xtol(self):
        ...
        try:
            ...
            QMessageBox.critical(
                self,
                "Invalid Input",
                "Please enter a valid number for mole fraction tolerance.",
            )
